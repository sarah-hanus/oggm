...
#    @pytest.mark.slow
    ...
    #    @pytest.mark.slow
    @pytest.mark.parametrize('store_daily_hydro', [False, True], ids=['annual', 'daily'])
    def test_hydro_out_daily_random_oggm_core(self, gdir, #inversion_params,
                                        store_daily_hydro):
            #TODO: need to make this test compatible !!!
            ...
            tasks.run_with_hydro_daily(gdir, run_task=run_random_climate_TIModel,
                                 store_daily_hydro=store_daily_hydro,
                                 seed=0, nyears=100, y0=2003 - 5, halfsize=5,
                                 output_filesuffix='_rand',
                                 melt_f='from_json',
                                 precipitation_factor=pf,
    ...
    def test_hydro_daily_vs_annual_from_oggm_core(self, gdir, #inversion_params,
        ...
        with xr.open_dataset(gdir.get_filepath('model_diagnostics',
            ...
            odf_ma = ds[sel_vars].mean(dim='time').to_dataframe()
            odf_ma.columns = [c.replace('_daily', '') for c in odf_ma.columns]
        ...
        assert odf_ma['melt_on_glacier'].loc[:90].mean() / odf_ma['melt_on_glacier'].loc[182:273].mean() < 0.02
        
        assert odf_ma['melt_on_glacier'].iloc[152:273].mean() > odf_ma['melt_on_glacier'].iloc[:90].mean()
        assert odf_ma['melt_on_glacier'].iloc[152:273].mean() > odf_ma['melt_on_glacier'].iloc[305:].mean()
        assert odf_ma['melt_off_glacier'].iloc[152:273].mean() > odf_ma['melt_off_glacier'].iloc[0:3].mean()
        assert odf_ma['melt_off_glacier'].iloc[152:273].mean() > odf_ma['melt_off_glacier'].iloc[305:].mean()
        assert odf_ma['liq_prcp_on_glacier'].iloc[152:273].mean() > odf_ma['liq_prcp_on_glacier'].iloc[:90].mean()
        assert odf_ma['liq_prcp_on_glacier'].iloc[152:273].mean() > odf_ma['liq_prcp_on_glacier'].iloc[305:].mean()
        assert odf_ma['liq_prcp_off_glacier'].iloc[152:273].mean() > odf_ma['liq_prcp_off_glacier'].iloc[0:3].mean()
        assert odf_ma['liq_prcp_off_glacier'].iloc[152:273].mean() > odf_ma['liq_prcp_off_glacier'].iloc[305:].mean()
        ...
        assert_allclose(odf_ma['runoff'].idxmax(), 227, atol=20)
        assert_allclose(odf_ma['melt_on_glacier'].idxmax(), 227, atol=20)
        #in summer month ratio of rain to snow should be largest
        ratio_rain_snow = odf_ma['liq_prcp_on_glacier'] / odf_ma['snowfall_on_glacier']
